...
namespace AppBancoDLL
{
    public class ComprasDAO
    {
        ...
        {
            string strQuery = string.Format("Insert into tbl_compra(cd_compra, cpf_usu, end_usu, numCasa_usu, blocoApa_usu, formaPag_usu)" +
                    "values('{0}', '{1}', '{2}', '{3}', '{4}', '{5}');", compra.cd_compra, compra.cpf_usu.Replace(".", string.Empty).Replace("-", string.Empty), compra.end_usu, compra.numCasa_usu, compra.blocoApa_usu, compra.formaPag_usu); ;
            using (db = new Banco())
            {
